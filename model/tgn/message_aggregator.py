...
class LastMessageAggregator(AbsMessageAggregator):
    ...
    def aggregate(
        ...
        unique_message_tensors = []
        unique_ts = []
        to_update_nodes = []

        for node_id in unique_nodes:
            message_list = messages[int(node_id)]
            if len(message_list) > 0:
                to_update_nodes.append(int(node_id))
                unique_message_tensors.append(message_list[-1].content)
                unique_ts.append(message_list[-1].ts)

        unique_message_tensors = torch.stack(unique_message_tensors) if len(to_update_nodes) > 0 else []
        unique_ts = torch.Tensor(unique_ts).float() if len(to_update_nodes) > 0 else []

        return torch.Tensor(to_update_nodes).long(), unique_message_tensors, unique_ts
