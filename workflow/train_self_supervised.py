...
from utils.training import RandomNodeSelector, load_model, save_model


def train_single_epoch(model: AbsModel, data: Dataset, batch_size: int, backprop_every: int) -> None:
...
def run_train_self_supervised(args: argparse.Namespace, config: dict) -> None:
    ...
    full_data, train_data, val_data, test_data, new_node_val_data, new_node_test_data, feature_repo = \
        get_self_supervised_data(logger=logger, workspace_path=workspace_path)
    ...
    for epoch in range(1, num_epoch + 1):
        ...
        train_single_epoch(model, train_data, train_batch_size, backprop_every)
